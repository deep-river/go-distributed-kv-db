...
func (s *Server) GetHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "called GET, Shard = %d, current shard = %d, addr = %q, Value = %q, error = %v", shard, s.shardIdx, s.addrs[shard], value, err)
}
...
func (s *Server) SetHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "called SET, error = %v, hash = %d, shardIdx = %d", err, shard, s.shardIdx)
}
